...
export default function Layout() {
  const { searchQuery, setSearchQuery, yearFilter, setYearFilter, salesYears, loading, error, refreshData } = useSalesData()
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      setUploading(true)
      setUploadError(null)
      await axios.post('http://127.0.0.1:5000/api/upload-data', formData, {
      ...
      <header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/85 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 py-3 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full lg:w-auto lg:min-w-[420px]">
            ...
            <label className="cursor-pointer rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white border border-cyan-500/60 px-4 py-2 text-sm font-medium transition-colors flex items-center justify-center min-w-[120px]">
        ...
        {uploadError && (
          <div className="max-w-7xl mx-auto px-4 pb-2 text-red-400 text-sm">{uploadError}</div>
        )}
        {loading && (
